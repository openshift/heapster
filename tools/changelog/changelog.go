...
var (
	...
	webconsoleBump = regexp.MustCompile(regexp.QuoteMeta("bump(heapster-web-console): ") + `([\w]+)`)
...
func main() {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		// has api changes
		display := fmt.Sprintf("%s [\\#%s](https://heapster/pull/%s)", message, matches[1], matches[1])
